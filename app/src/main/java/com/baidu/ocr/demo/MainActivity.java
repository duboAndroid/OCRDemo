...
public class MainActivity extends AppCompatActivity {
    ...
    private void initAccessTokenWithAkSk() {
        ...
        }, getApplicationContext(),  "T3c7pojo7c1G5t0LwkY8ctDv", "033UF7SGHvIpsp5T8SdAMYw1weEw2v0g");
    }
    ...
    private void alertText(final String title, final String message) {
        this.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                List<TestBean.WordsResultBean> wordsResultBeanList = new ArrayList<>();
                String strWord = "";
                try {
                    ...
                    if(null != words){
                        for(int i =0;i < words.length();i++){
                            JSONObject jObject3=words.getJSONObject(i);
                            //strWord += jObject3.getString("words");
                            TestBean.WordsResultBean wordsResultBean = new TestBean.WordsResultBean();
                            wordsResultBean.setWords(jObject3.getString("words"));
                            wordsResultBeanList.add(wordsResultBean);
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
                if(null != wordsResultBeanList && wordsResultBeanList.size() >0){
                    for(int i =0;i < wordsResultBeanList.size();i++){
                        strWord+=wordsResultBeanList.get(i).getWords();
                    }
                }
                final String finalStrWord = strWord;
                if(TextUtils.isEmpty(finalStrWord)){
                ...
                alertDialog.setTitle(title)
                        ...
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                // 创建普通字符型ClipData
                                ClipData mClipData = ClipData.newPlainText("Label", finalStrWord);
                                // 将ClipData内容放到系统剪贴板里。
